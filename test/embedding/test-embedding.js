...
function runTests(testRunner) {
  spawnSyncAndAssert(
    binary,
    ['console.log(42)', testRunner],
    {
      trim: true,
      stdout: '42',
    });
  ...
  spawnSyncAndAssert(
    binary,
    ['console.log(embedVars.nön_ascıı)', testRunner],
    {
      trim: true,
      stdout: '🏳️‍🌈',
    });
  ...
  spawnSyncAndExit(
    binary,
    ['require("lib/internal/test/binding")', testRunner],
    {
      status: 1,
      signal: null,
    });
  ...
  spawnSyncAndExit(
    binary,
    [`require(${fixturePath})`, 92, testRunner],
    {
      status: 92,
      signal: null,
    });
...
runTests('');
runTests('--node-api');
//const node_api_tests = require('./test-node-api-embedding');
//node_api_tests.runTests(binary);