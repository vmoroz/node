  ...
  env->TryLoadAddon(*filename, flags, [&](DLib* dlib) {
    ...
    if (mp != nullptr) {
      ...
      if (auto callback = GetInitializerCallback(dlib)) {
        ...
        return true;
      } else if (napi_addon_register_func init_module = GetNodeApiInitializerCallback(dlib)) {
        int32_t module_api_version = NODE_API_DEFAULT_MODULE_API_VERSION;
        if (node_api_addon_get_api_version_func get_version = GetNodeApiAddonGetApiVersionCallback(dlib)) {
          module_api_version = get_version();
        }
        if (node_api_addon_set_vtable_func set_vtable = GetNodeApiAddonSetVtableCallback(dlib)) {
