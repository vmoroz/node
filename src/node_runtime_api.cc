...
#define EMBEDDED_RUNTIME(rt_env) CAST_NOT_NULL_TO(rt_env, EmbeddedRuntime)
...
class NodeFunctor<TResult (*)(void*, TArgs...)> {
 ...
 public:
  ...
  NodeFunctor(TCallback callback,
              void* data,
              node_rt_data_release_callback data_release)
  ...
  node_rt_data_release_callback data_release() const {
 ...
 private:
  NodePointer<TCallback> callback_;
  NodePointer<void*> data_;
  NodePointer<node_rt_data_release_callback> data_release_;
};
...
class NodeErrorInfo {
 public:
  static const char* GetLastErrorMessage() {
    return node_rt_last_error_message_get();
  }

  static void SetLastErrorMessage(const char* message) {
    return node_rt_last_error_message_set(message);
  }
  ...
  static void ClearLastErrorMessage() {
    node_rt_last_error_message_set(nullptr);
  }
...
std::shared_ptr<NodeFunctor<TCallback>> MakeSharedFunctorPtr(
    TCallback callback,
    void* callback_data,
    node_rt_data_release_callback release_callback_data) {
...
class EmbeddedPlatform {
 public:
  ...
  NodeStatus Initialize(node_rt_platform_configure_callback configure_platform,
                        void* configure_platform_data,
                        bool* early_return);
...
class EmbeddedRuntime {
 public:
  ...
  static NodeStatus Run(node_rt_platform platform,
                        node_rt_env_configure_callback configure_env,
                        void* configure_env_data);

  static NodeStatus Create(node_rt_platform platform,
                           node_rt_env_configure_callback configure_env,
                           void* configure_env_data,
                           node_rt_env* result);
  ...
  NodeStatus OnPreload(node_rt_env_preload_callback run_preload,
                       void* preload_data,
                       node_rt_data_release_callback release_preload_data);

  NodeStatus OnStartExecution(
      node_rt_env_loading_callback start_execution,
      void* start_execution_data,
      node_rt_data_release_callback release_start_execution_data);

  NodeStatus OnHandleExecutionResult(
      node_rt_env_loaded_callback handle_result,
      void* handle_result_data,
      node_rt_data_release_callback release_handle_result_data);

  NodeStatus AddModule(const char* module_name,
  ...
  NodeStatus Initialize(node_rt_env_configure_callback configure_env,
                        void* configure_env_data);
  ...
  NodeStatus SetUserData(void* user_data,
  ...
  NodeStatus RunNodeApi(node_rt_node_api_run_callback run_node_api,
                        void* run_node_api_data);

  NodeStatus OpenNodeApiScope(node_rt_node_api_scope* node_api_scope,
                              napi_env* env);
  NodeStatus CloseNodeApiScope(node_rt_node_api_scope node_api_scope);
 ...
 private:
  ...
  struct V8ScopeLocker {
    explicit V8ScopeLocker(EmbeddedRuntime& env)
        : env_(env), nest_level_(env_.OpenV8Scope()) {}

    ~V8ScopeLocker() { env_.CloseV8Scope(nest_level_); }

    V8ScopeLocker(const V8ScopeLocker&) = delete;
    V8ScopeLocker& operator=(const V8ScopeLocker&) = delete;

   private:
    EmbeddedRuntime& env_;
    size_t nest_level_;
  };
  ...
  class ReleaseCallbackDeleter {
   public:
    explicit ReleaseCallbackDeleter(
        node_rt_data_release_callback release_callback)
   ...
   private:
    node_rt_data_release_callback release_callback_;
  };
 ...
 private:
  ...
  // The node API associated with the env's environment.
...
NodeStatus EmbeddedPlatform::RunMain(
  ...
  return EmbeddedRuntime::Run(platform, configure_env, configure_env_data);
}
...
/*static*/ NodeStatus EmbeddedPlatform::Create(
  ...
  *result = reinterpret_cast<node_rt_platform>(platform_ptr.release());
  return NodeStatus::kOk;
}
...
NodeStatus EmbeddedPlatform::Initialize(
  ...
  node_rt_platform_config platform_config =
      reinterpret_cast<node_rt_platform_config>(this);
  ...
  c_env_args_ = NodeCStringArray<>(init_result_->exec_args());
...
NodeStatus EmbeddedPlatform::GetParsedArgs(int32_t* args_count,
  ...
  if (env_args_count != nullptr) {
    *env_args_count = c_env_args_.size();
  }
  if (env_args != nullptr) {
    *env_args = c_env_args_.c_strs();
  }
  return NodeStatus::kOk;
}
...
/*static*/ NodeStatus EmbeddedRuntime::Create(
  ...
  std::unique_ptr<EmbeddedRuntime> env_ptr =
      std::make_unique<EmbeddedRuntime>(platform_ptr);

  CHECK_STATUS(env_ptr->Initialize(configure_env, configure_env_data));

  *result = reinterpret_cast<node_rt_env>(env_ptr.release());
...
NodeStatus EmbeddedRuntime::OnPreload(
    node_rt_env_preload_callback run_preload,
    void* preload_data,
    node_rt_data_release_callback release_preload_data) {
  ASSERT_EXPR(!is_initialized_);
         ...
         run_preload_ptr = MakeSharedFunctorPtr(
          ...
          env->CallIntoModule(
              [&](napi_env env) {
                ...
                (*run_preload_ptr)(reinterpret_cast<node_rt_env>(this),
                                   env,
                                   process_value,
                                   require_value);
              },
         ...
         start_execution_ptr =
          ...
          node_api_env_->CallIntoModule(
              [&](napi_env env) {
                ...
                result =
                    (*start_execution_ptr)(reinterpret_cast<node_rt_env>(this),
                                           env,
                                           process_value,
                                           require_value,
                                           run_cjs_value);
...
NodeStatus EmbeddedRuntime::AddModule(
  ...
  auto insert_result = modules_.try_emplace(
      module_name,
      ModuleInfo{reinterpret_cast<node_rt_env>(this),
                 module_name,
...
NodeStatus EmbeddedRuntime::Initialize(
    node_rt_env_configure_callback configure_env, void* configure_env_data) {
  ASSERT_EXPR(!is_initialized_);

  if (configure_env != nullptr) {
    CHECK_STATUS(configure_env(configure_env_data,
                               reinterpret_cast<node_rt_platform>(platform_),
                               reinterpret_cast<node_rt_env_config>(this)));
  }
  ...
  if (handle_result_) {
    node_api_env_->CallIntoModule(
        [&](napi_env env) {
          handle_result_(reinterpret_cast<node_rt_env>(this),
                         env,
...
NodeStatus EmbeddedRuntime::SetUserData(
    void* user_data, node_rt_data_release_callback release_user_data) {
...
void EmbeddedRuntime::RunPollingThread(void* data) {
  EmbeddedRuntime* env = static_cast<EmbeddedRuntime*>(data);
  for (;;) {
    // Wait for the task runner to deal with events.
    uv_sem_wait(&env->polling_sem_);
    if (env->polling_thread_closed_) break;
    ...
    NodeStatus post_result = env->post_task_(
        [](void* task_data) {
          auto* env = static_cast<EmbeddedRuntime*>(task_data);
          return env->RunEventLoopNoWait(nullptr);
        },
        env,
        nullptr,
        &succeeded);
...
NodeStatus EmbeddedRuntime::RunNodeApi(
    node_rt_node_api_run_callback run_node_api, void* run_node_api_data) {
  ASSERT_ARG_NOT_NULL(run_node_api);

  node_rt_node_api_scope node_api_scope{};
  napi_env env{};
...
NodeStatus EmbeddedRuntime::OpenNodeApiScope(
    node_rt_node_api_scope* node_api_scope, napi_env* env) {
  ...
  *node_api_scope =
      reinterpret_cast<node_rt_node_api_scope>(node_api_scope_data_.size());
  *env = node_api_env_;
  return NodeStatus::kOk;
}
...
napi_env EmbeddedRuntime::GetOrCreateNodeApiEnv(
  ...
  // Check if this is the main environment associated with the env.
  ...
  if (insert_result.second) {
    ...
    node_env->AddCleanupHook(
        [](void* arg) {
          ...
          std::scoped_lock<std::mutex> lock(context->env_->worker_env_mutex_);
          context->env_->worker_env_to_node_api_.erase(context->node_env_);
        },
...
const char* NAPI_CDECL node_rt_last_error_message_get() {
  return EmbeddedErrorHandling::GetLastErrorMessage();
}

void NAPI_CDECL node_rt_last_error_message_set(const char* message) {
  if (message == nullptr) {
...
void NAPI_CDECL node_rt_last_error_message_set_format(const char* format, ...) {
...
node_rt_status NAPI_CDECL node_rt_platform_delete(node_rt_platform platform) {
  return EMBEDDED_PLATFORM(platform)->DeleteMe();
}

node_rt_status NAPI_CDECL node_rt_platform_config_set_flags(
...
node_rt_platform_get_parsed_args(node_rt_platform platform,
  ...
  return EMBEDDED_PLATFORM(platform)->GetParsedArgs(
      args_count, args, env_args_count, env_args);
}
...
node_rt_env_create(node_rt_platform platform,
  ...
  return EmbeddedRuntime::Create(
      platform, configure_env, configure_env_data, result);
}

node_rt_status NAPI_CDECL node_rt_env_delete(node_rt_env rt_env) {
  return EMBEDDED_RUNTIME(rt_env)->DeleteMe();
}

node_rt_status NAPI_CDECL node_rt_env_config_set_node_api_version(
    node_rt_env_config env_config, int32_t node_api_version) {
  return EMBEDDED_RUNTIME(env_config)->SetNodeApiVersion(node_api_version);
}

node_rt_status NAPI_CDECL node_rt_env_config_set_flags(
    node_rt_env_config env_config, node_rt_env_flags flags) {
  return EMBEDDED_RUNTIME(env_config)->SetFlags(flags);
}
...
node_rt_env_config_set_args(node_rt_env_config env_config,
                            ...
                            int32_t env_argc,
                            const char* env_argv[]) {
  return EMBEDDED_RUNTIME(env_config)->SetArgs(argc, argv, env_argc, env_argv);
}

node_rt_status NAPI_CDECL node_rt_env_config_on_preload(
    node_rt_env_config env_config,
    node_rt_env_preload_callback run_preload,
    void* preload_data,
    node_rt_data_release_callback release_preload_data) {
  return EMBEDDED_RUNTIME(env_config)
      ->OnPreload(run_preload, preload_data, release_preload_data);
}

node_rt_status NAPI_CDECL node_rt_env_config_on_loading(
    node_rt_env_config env_config,
    node_rt_env_loading_callback start_execution,
    void* start_execution_data,
    node_rt_data_release_callback release_start_execution_data) {
...
node_rt_status NAPI_CDECL node_rt_env_config_on_loaded(
    node_rt_env_config env_config,
    node_rt_env_loaded_callback handle_result,
    void* handle_result_data,
    node_rt_data_release_callback release_handle_result_data) {
...
node_rt_status NAPI_CDECL node_rt_env_config_add_module(
    node_rt_env_config env_config,
    const char* module_name,
    node_rt_module_initialize_callback init_module,
    void* init_module_data,
...
node_rt_env_user_data_set(node_rt_env rt_env,
                          void* user_data,
                          node_rt_data_release_callback release_user_data) {
  return EMBEDDED_RUNTIME(rt_env)->SetUserData(user_data, release_user_data);
}

node_rt_status NAPI_CDECL node_rt_env_user_data_get(node_rt_env rt_env,
                                                    void** user_data) {
  return EMBEDDED_RUNTIME(rt_env)->GetUserData(user_data);
}

node_rt_status NAPI_CDECL node_rt_env_config_set_task_runner(
    node_rt_env_config env_config,
    node_rt_task_post_callback post_task,
    void* post_task_data,
    node_rt_data_release_callback release_post_task_data) {
...
node_rt_status NAPI_CDECL node_rt_env_event_loop_run(node_rt_env rt_env) {
  return EMBEDDED_RUNTIME(rt_env)->RunEventLoop();
}

node_rt_status NAPI_CDECL node_rt_env_event_loop_terminate(node_rt_env rt_env) {
  return EMBEDDED_RUNTIME(rt_env)->TerminateEventLoop();
}

node_rt_status NAPI_CDECL node_rt_env_event_loop_run_once(node_rt_env rt_env,
                                                          bool* has_more_work) {
  return EMBEDDED_RUNTIME(rt_env)->RunEventLoopOnce(has_more_work);
}

node_rt_status NAPI_CDECL
node_rt_env_event_loop_run_no_wait(node_rt_env rt_env, bool* has_more_work) {
  return EMBEDDED_RUNTIME(rt_env)->RunEventLoopNoWait(has_more_work);
}
...
node_rt_env_node_api_run(node_rt_env rt_env,
                         ...
                         void* run_node_api_data) {
  return EMBEDDED_RUNTIME(rt_env)->RunNodeApi(run_node_api, run_node_api_data);
}

node_rt_status NAPI_CDECL node_rt_env_node_api_scope_open(
    node_rt_env rt_env, node_rt_node_api_scope* node_api_scope, napi_env* env) {
  return EMBEDDED_RUNTIME(rt_env)->OpenNodeApiScope(node_api_scope, env);
}

node_rt_status NAPI_CDECL node_rt_env_node_api_scope_close(
    node_rt_env rt_env, node_rt_node_api_scope node_api_scope) {
  return EMBEDDED_RUNTIME(rt_env)->CloseNodeApiScope(node_api_scope);
}
