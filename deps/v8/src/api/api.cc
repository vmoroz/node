...
v8::Local<v8::Object> v8::Object::NewAsJson(Isolate* v8_isolate,
 ...
 auto parentOfDescriptorOwner = [](i::Isolate* isolate, i::Handle<i::Map> maybe_root,
  ...
  for (i = 0; i < length; i++) {
    ...
    i::Representation expected_representation = details.representation();
    ...
    if (!value->FitsRepresentation(expected_representation)) {
      ...
      i::Handle<i::FieldType> value_type =
          value->OptimalType(i_isolate, representation);
      i::MapUpdater::GeneralizeField(i_isolate, target, descriptor_index,
                                  details.constness(), representation,
                                  value_type);
    } else if (expected_representation.IsHeapObject() &&
               !target->instance_descriptors(i_isolate)
                    .GetFieldType(descriptor_index)
                    .NowContains(value)) {
      i::Handle<i::FieldType> value_type =
          value->OptimalType(i_isolate, expected_representation);
      i::MapUpdater::GeneralizeField(i_isolate, target, descriptor_index,
  ...
  i::Handle<i::ByteArray> mutable_double_buffer;
  // Allocate enough space so we can double-align the payload.
  const int kMutableDoubleSize = sizeof(double) * 2;
  static_assert(i::HeapNumber::kSize <= kMutableDoubleSize);
  if (new_mutable_double > 0) {
    mutable_double_buffer =
        i_isolate->factory()->NewByteArray(kMutableDoubleSize * new_mutable_double);
  }

  i::Handle<i::JSObject> object = initial_map->is_dictionary_map()
                                ? i_isolate->factory()->NewSlowJSObjectFromMap(map)
                                : i_isolate->factory()->NewJSObjectFromMap(map);
  object->set_elements(*elements);

  {
    descriptor = 0;
    i::DisallowGarbageCollection no_gc;
    i::WriteBarrierMode mode = object->GetWriteBarrierMode(no_gc);
    i::Address mutable_double_address =
        mutable_double_buffer.is_null()
            ? 0
            : reinterpret_cast<i::Address>(
                  mutable_double_buffer->GetDataStartAddress());
    i::Address filler_address = mutable_double_address;
    if (!V8_COMPRESS_POINTERS_8GB_BOOL && i::kTaggedSize != i::kDoubleSize) {
      if (IsAligned(mutable_double_address, i::kDoubleAlignment)) {
        mutable_double_address += i::kTaggedSize;
      } else {
      ...
      if (details.representation().IsDouble()) {
        if (value->IsSmi()) {
          if (!V8_COMPRESS_POINTERS_8GB_BOOL && i::kTaggedSize != i::kDoubleSize) {
            ...
            i::HeapObject filler = i::HeapObject::FromAddress(filler_address);
            filler.set_map_after_allocation(
                *i_isolate->factory()->one_pointer_filler_map());
          ...
          *value = hn;
          ...
          i::HeapObject::cast(*value).set_map(*i_isolate->factory()->heap_number_map(),
                                          kReleaseStore);
        }
      }
    ...
    if (!mutable_double_buffer.is_null()) {
#ifdef DEBUG
      i::Address end =
          reinterpret_cast<i::Address>(mutable_double_buffer->GetDataEndAddress());
      if (!V8_COMPRESS_POINTERS_8GB_BOOL && i::kTaggedSize != i::kDoubleSize) {
      ...
      i_isolate->heap()->EnsureSweepingCompletedForObject(
          *mutable_double_buffer);
